...
class CachedTodoistAPI(TodoistAPI):
    ...
    # capture_list will convert a PagintedResults object into a 2D list
    # this means that the return type is now List[List[Type]] instead of Iterator[List[Type]]
    ...
    # Task manipulators
    ###############################################
    add_task                                = cache_invalidator(TodoistAPI.add_task,         TASK_SENSITIVE_CACHES)
    add_task_quick                          = cache_invalidator(TodoistAPI.add_task_quick,   TASK_SENSITIVE_CACHES)
    update_task                             = cache_invalidator(TodoistAPI.update_task,      TASK_SENSITIVE_CACHES)
    complete_task                           = cache_invalidator(TodoistAPI.complete_task,    TASK_SENSITIVE_CACHES)
    uncomplete_task                         = cache_invalidator(TodoistAPI.uncomplete_task,  TASK_SENSITIVE_CACHES)
    move_task                               = cache_invalidator(TodoistAPI.move_task,        TASK_SENSITIVE_CACHES)
    delete_task                             = cache_invalidator(TodoistAPI.delete_task,      TASK_SENSITIVE_CACHES | COMMENT_SENSITIVE_CACHES | LABEL_SENSITIVE_CACHES)

    ###############################################
    # Section manipulators
    ###############################################
    add_section                             = cache_invalidator(TodoistAPI.add_section,       SECTION_SENSITIVE_CACHES)
    update_section                          = cache_invalidator(TodoistAPI.update_section,    SECTION_SENSITIVE_CACHES)
    delete_section                          = cache_invalidator(TodoistAPI.delete_section,    SECTION_SENSITIVE_CACHES | TASK_SENSITIVE_CACHES | COMMENT_SENSITIVE_CACHES | LABEL_SENSITIVE_CACHES)

    ###############################################
    # Project manipulators
    ###############################################
    add_project                             = cache_invalidator(TodoistAPI.add_project,       PROJECT_SENSITIVE_CACHES)
    update_project                          = cache_invalidator(TodoistAPI.update_project,    PROJECT_SENSITIVE_CACHES)
    archive_project                         = cache_invalidator(TodoistAPI.archive_project,   PROJECT_SENSITIVE_CACHES)
    unarchive_project                       = cache_invalidator(TodoistAPI.unarchive_project, PROJECT_SENSITIVE_CACHES)
    delete_project                          = cache_invalidator(TodoistAPI.delete_project,    PROJECT_SENSITIVE_CACHES | SECTION_SENSITIVE_CACHES | TASK_SENSITIVE_CACHES | COMMENT_SENSITIVE_CACHES | LABEL_SENSITIVE_CACHES)

    ###############################################
    # Comment manipulators
    ###############################################
    add_comment                             = cache_invalidator(TodoistAPI.add_comment,      COMMENT_SENSITIVE_CACHES)
    update_comment                          = cache_invalidator(TodoistAPI.update_comment,   COMMENT_SENSITIVE_CACHES)
    delete_comment                          = cache_invalidator(TodoistAPI.delete_comment,   COMMENT_SENSITIVE_CACHES)

    ###############################################
    # Label manipulators
    ###############################################
    add_label                               = cache_invalidator(TodoistAPI.add_label,              LABEL_SENSITIVE_CACHES)
    update_label                            = cache_invalidator(TodoistAPI.update_label,           LABEL_SENSITIVE_CACHES)
    delete_label                            = cache_invalidator(TodoistAPI.delete_label,           LABEL_SENSITIVE_CACHES)
    rename_shared_label                     = cache_invalidator(TodoistAPI.rename_shared_label,    LABEL_SENSITIVE_CACHES)
    remove_shared_label                     = cache_invalidator(TodoistAPI.remove_shared_label,    LABEL_SENSITIVE_CACHES)
